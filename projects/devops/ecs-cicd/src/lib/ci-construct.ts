...
export class CIConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: CIConstructProps) {
    ...
    githubPipeline.addStage({ stageName: 'GithubSource' }).addAction(githubSourceAction);
    ...
    const cfnGitHubWebhook = new codepipeline.CfnWebhook(this, 'CfnGitHubWebhook', {
      ...
      authenticationConfiguration: {
        secretToken: secretsmanager.Secret.fromSecretNameV2(this, 'GithubPAT',
          props.secretKey).secretValue.toString(),
      },
      filters: [{
        jsonPath: '$.ref',
    ...
    codecommitPipeline.addStage({ stageName: 'CodeCommitSource' }).addAction(CodeCommitSourceAction);
    codecommitPipeline.addStage({ stageName: 'DockerImageBuild' }).addAction(buildAction);
    ...
    new CDConstruct(this, 'CDFromGithub', {
      imageTag: 'latest',
