...
export class CIGithubConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: CIConstructProps) {
    ...
    pipeline.addStage({ stageName: 'GithubSource' }).addAction(githubSourceAction);
    pipeline.addStage({ stageName: 'ImageBuild', actions: [buildAction] });
